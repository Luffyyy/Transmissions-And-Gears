...
namespace GearFactory
{
    ...
    public class Gear : GearBase
    {
        ...
        protected void BuildMeshComponents()
        {
            ...
            float rootAngleShift = Mathf.Max(Mathf.Abs(angleDelta * rootAngleShiftMultiplier), 0.01f) * Mathf.Sign(rootAngleShiftMultiplier);
            float outerAngleShift = Mathf.Max(Mathf.Abs(angleDelta * outerAngleShiftMultiplier), 0.01f) * Mathf.Sign(outerAngleShiftMultiplier);
            float tipAngleShift = Mathf.Max(Mathf.Abs(angleDelta * tipAngleShiftMultiplier), 0.01f) * Mathf.Sign(tipAngleShiftMultiplier);
            float _teethSlant = teethSlant * angleDelta;
            ...
            {
                float angle = 2 * i * angleDelta + angleShift + outerAngleShift + _teethSlant;
                vertices.Add(new Vector3(Mathf.Cos(angle), Mathf.Sin(angle)) * outerRadius);
                angle = (2 * i + 1) * angleDelta + angleShift - outerAngleShift + _teethSlant;
                vertices.Add(new Vector3(Mathf.Cos(angle), Mathf.Sin(angle)) * outerRadius);
            }
            ...
            for (int i = 0; i < sides; i++)
            {
                float angleLeft = 2 * i * angleDelta + angleShift + tipAngleShift + _teethSlant * 2;
                vertices.Add(new Vector3(Mathf.Cos(angleLeft), Mathf.Sin(angleLeft)) * tipRadius);

                float angleRight = (2 * i + 1) * angleDelta + angleShift - tipAngleShift + _teethSlant * 2;
