    ...
    public class GearCreatorEditor : Editor
    {
        ...
        private static void Build(GearCreator gc)
        {
            Gear createdGear = Gear.AddGear(gc.transform.position, gc.gearSides, gc.gearInnerRadius,
                gc.actualRootRadius, gc.gearRootAngleShiftMultiplier, gc.actualOuterRadius,
                gc.gearOuterAngleShiftMultiplier, gc.actualTipRadius,
            ...
            if (gc.thickness > 0)
            {
                createdGear.ConvertTo3D(gc.thickness);
            }
